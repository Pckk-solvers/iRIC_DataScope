...
class ProjectPathSelector(ttk.Frame):
    """
    入力: プロジェクトフォルダ / CSVフォルダ / .ipro を選択できるセレクタ。
    """
    ...
    def get_path(self) -> Path:
        """現在の入力パスを Path で返す（フォルダ or .ipro）"""
        return Path(self.var.get())


class IOFolderSelector(ttk.Frame):
    """
    入力（プロジェクトフォルダ/CSVフォルダ/.ipro）と出力フォルダを一度に選択・取得できるウィジェット。
