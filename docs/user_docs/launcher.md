...
入力パス（プロジェクトフォルダ / CSV フォルダ / `.ipro` / `.cgn`）と出力フォルダを指定し、各機能（別ウィンドウ）を起動するための入口です。
...
## 使い方（最短）
1. 「入力パス」でプロジェクトフォルダ / `.ipro` / `.cgn` / CSV フォルダを選択
2. 「出力フォルダ」で保存先を選択
...
## 画面の見方
- **入力診断**: 入力パスを自動判定し、`Case1.cgn` / `Solution*.cgn` / `Result_*.csv` の検出状況を表示します。
