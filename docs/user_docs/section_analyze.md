...
2. **集計設定**
   ...
   - **断面ID属性（任意）**: 未指定なら `SEC001...` を自動採番します。
   - **断面名属性（必須）**: タイトルの `{section_name}` に使います。

3. **グラフ設定**
   - **グラフY軸**: 断面ごと最適、または全断面共通スケールを選べます。
   - **横軸目盛間隔[h]**: 空欄なら自動目盛、入力時は固定刻みです。
   - **縦軸目盛間隔**: 空欄なら自動目盛、入力時は固定刻みです。
...
6. **実行**
   - **断面集計を実行**: 全断面を処理します。
   - **1断面プレビュー**: 1断面だけメモリ上で描画し、見た目を確認できます（CSV/PNGは出力しません）。
   - プレビューはDPI設定を反映しません。解像度確認は出力PNGで行ってください。
...
- `タイトル形式` は文字テンプレートです。  
  例: `{section_id} {section_name} / 平均水位時系列`
- `{section_id}` は **集計設定の「断面ID属性（任意）」** で決まります。  
  未指定なら `SEC001...` を自動採番します。
- `{section_name}` は **集計設定の「断面名属性（必須）」** で決まります。
- 期待どおりに出ない場合は、まず「断面名属性」が正しい列かを確認してください。
