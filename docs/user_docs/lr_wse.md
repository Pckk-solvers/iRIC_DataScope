...
![左右岸水位抽出（説明付き）](../images/lr_wse_ui_annotated.png)
...
## 画面説明（番号）
1. 設定ファイルの選択（`setting.csv`）
2. 中間 CSV 出力の有無（必要なら自動で出力先を設定）
3. 出力ファイル名の指定
4. 欠損値置換（空欄なら空セル）
5. 最小水深（`depth(m)` が入力値以下の場合、該当岸は「取得不可」）
6. 実行ボタンで処理開始
...
> KP は `97.6k` のような末尾 `k/K` 付きでも読み込めます。
