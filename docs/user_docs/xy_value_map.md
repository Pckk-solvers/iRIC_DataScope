...
1. **左パネル**  
   ...
   - 出力ステップ範囲、余白、フォントサイズ、出力ボタン（単ステップ / 全ステップ）
...
4. **色とスケールを設定**  
   ...
   - スケール: `global`（ROI 内全ステップの min/max をバックグラウンド計算）または `manual`（vmin/vmax を入力）  
   - `manual` の場合はレンジスライダーでも調整できます。自動計算値を範囲として保持します。

5. **解像度・出力オプションを調整**  
   - 解像度倍率（resolution）は推定格子間隔を基準に dx/dy をスケーリング  
   - 目盛り、枠線、カラーバー、タイトル、カラーバー名、フォントサイズ、`pad_inches`、出力倍率 (dpi スケール) を指定可能

6. **出力ステップ範囲を設定**  
   - 開始/終了ステップと間引き (`skip`) を入力またはレンジスライダーで指定  
   - 範囲外の値は自動で補正されます。

7. **出力を実行**  
   - `現在のステップを出力`: プレビュー中のステップのみ PNG 出力  
   - `全ステップを出力`: 指定範囲（開始/終了/skip）で連続出力。進捗バーとログを表示します。
...
| オプション | 説明 |
| ---------- | ---- |
| Show Title | チェック固定。文字列を空にすると非表示 |
| Title / Colorbar Label | 任意文字列。空なら非表示 |
| Show Ticks / Frame / Colorbar | それぞれオン/オフ可能 |
| Font Size | タイトル / 目盛り / カラーバーラベルを個別に指定 |
| Pad (in) | `bbox_inches="tight"` に追加する余白。プレビューと出力で共通 |
| Output Scale | プレビューの DPI を倍率変更して出力解像度を調整 |
