...
## 目的
プロジェクトフォルダ / `.ipro` / iRIC 出力 CSV（`Result_*.csv`）から、横断データを読み込み、標高/水位等の重ね合わせ図を Excel に出力する。
...
## 前提（入力列）
CSV/CGNS から読み込んだデータに、以下の列が存在する想定です（名称は iRIC 側の出力に依存します）。
...
- `iRIC_DataScope/cross_section/data_loader.py`
  - CSVフォルダ: CSV を読み込み、Profile ID（`I`）とソートキー（`J`）でグループ化
  - プロジェクトフォルダ / `.ipro`: CGNS を直接読み込み、同様にグループ化
