...
- 入力が `.ipro` / `.cgn` の場合、**既存ツールが必要とする `Result_*.csv` に自動変換してから**処理を継続できるようにする

## 入力/出力
- 入力: フォルダ、または `.ipro` / `.cgn`
  - フォルダ入力: 既に `Result_*.csv` が存在する前提（従来の iRIC 出力 CSV）
  - `.ipro` / `.cgn`: 起動前に `Result_*.csv` に変換してから各機能へ渡す（下記「変換」参照）
...
## 変換（`.ipro` / `.cgn` → `Result_*.csv`）
### 対象
- `左右岸水位抽出` / `横断重ね合わせ図作成` / `時系列データ抽出` の3機能は、入力が `.ipro` / `.cgn` の場合に変換を挟む
- `X-Y分布画像出力` は `.ipro` / `.cgn` / CSVフォルダを **直接読み込む**（CSV変換は不要）

### 変換の出力先
- 出力フォルダ配下に `converted_<入力stem>/` を作り、そこに `Result_*.csv` を生成する
...
### ユーザ通知（ダイアログ）
- 変換中: 進捗ウィンドウを表示（入力が `.ipro` の場合と `.cgn` の場合で文言を切り替え）
- 完了/失敗: ダイアログで通知
