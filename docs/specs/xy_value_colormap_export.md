...
- 回転: **ROI 中心**を基準に座標を回転して表示する
- 解像度: `dx/dy` を目安に I/J 補間倍率を決定（おおよその実距離に合わせる）
- 補間: SciPy の `ndimage.zoom` により X/Y/Value を I/J 方向に補間する
- 格子形状: X/Y を補間して使うため、元の曲線格子の形状を保つ
...
- 画像出力: `iRIC_DataScope/xy_value_map/main.py`
  ...
  - 回転角度と `dx/dy` を反映した I/J 補間結果を出力する
