...
## 目的
iRIC 出力 CSV（`Result_*.csv`）を全ステップ走査し、指定した格子点（`I,J`）における指定変数の時系列を抽出して Excel に出力する。
...
## 処理の流れ（概要）
- `iRIC_DataScope/common/csv_reader.py`
  - `list_csv_files()` で `Result_*.csv` を列挙
  - `read_iric_csv()` で時刻 `t` と DataFrame を取得
- `iRIC_DataScope/time_series/processor.py`
  - `aggregate_all()` が全 CSV を処理し、格子点ごとの時系列 DataFrame を作成
