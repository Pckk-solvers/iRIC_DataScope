...
- `iRIC_DataScope/common/cgns_reader.py`
  - `.ipro`/`.cgn`/フォルダ入力から CGNS を解決（`.ipro` は ZIP から一時展開）
...
（現在は実装済み）
- 入力 UI は「フォルダ or `.ipro`/`.cgn`」を受け付ける。
- 各機能起動前に、入力が `.ipro`/`.cgn` の場合は CSV 変換を実行し、変換後フォルダを各 GUI の入力として渡す。
  - 変換先: 出力フォルダ配下の `converted_<stem>/`
